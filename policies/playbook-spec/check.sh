...
if [ -d flows ]; then
  for path in $(find flows -maxdepth 1 -type f -name '*.md' | sort); do
    case "$path" in
      flows/index.md) continue ;;
    esac
...
"
    ...
    for field in title summary created last_updated; do
      value="$(field_value "$path" "$field" || true)"
      if [ -z "$value" ]; then
        violations="${violations}${path}: missing front matter field '${field}'
"
...
printf 'Playbook and flow front matter is valid.\n'
