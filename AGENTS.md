...
through one stable command: `just test`.
...
- Use `just test` as the repository verification entrypoint.
...
| Class 1 | Docs, indexes, playbooks, trace files, or markdown metadata only. | Run focused checks for touched files, then `just test` before handoff when practical. |
| Class 2 | Repository governance, policies, AGENTS or CLAUDE instructions, scripts, CI, or shared workflow tooling. | Run focused checks and `just test`. |
| Class 3 | Consumer project production code, release behavior, security-sensitive workflow, migrations, dependencies, or user-facing behavior. | Run focused checks, `just test`, and any domain-specific verification named by the relevant playbook or consuming repo. |
...
- `just test`: run all repository checks.
- `just policies`: run policy checks directly.

If this template is copied into another repository, preserve the `just test`
contract and adapt the playbooks before adding language-specific commands.
